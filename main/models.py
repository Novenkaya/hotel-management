...
class Customer(models.Model):
    ...
    def get_absolute_url(self):
        """
Он создает URL для деталей клиента.
'customer-detail' - это имя URL.
Customer_id принимает аргумент
        """
...
class Room(models.Model):
    ...
    def display_facility(self):
        """
        Эта функция должна быть определена как объект имеет много атрибутов
List_display не может быть отображен непосредственно на панели администратора
        """
    ...
    def save(self, *args, **kwargs):  # По умолчанию Сохранить поведение
        if self.reservation:  # Если он был сохранен, он не должен быть доступен
            self.availability = 0
        else:
            self.availability = 1
...
class Facility(models.Model):
    ...
    class Meta:
        verbose_name_plural = 'Facilities'  # В противном случае на панели администратора отображаются объекты
